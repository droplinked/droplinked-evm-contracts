...
describe('BulkTokenDistributor', function () {
	...
	before(async function () {
		...
		this.erc20 = await ERC20Mock.deploy(
			'TestToken',
			'TT',
		...
		this.erc1155 = await ERC1155Mock.deploy();
	});
	...
	describe('ERC20 Distribution', function () {
		it('Should distribute ERC20 tokens correctly', async function () {
			...
			// Check balances
			expect(
				await this.erc20.balanceOf(await addr1.getAddress())
			).to.equal(100);
			expect(
				await this.erc20.balanceOf(await addr2.getAddress())
			).to.equal(200);
			expect(
				await this.erc20.balanceOf(await addr3.getAddress())
			).to.equal(300);
		});
	...
	describe('ERC721 Distribution', function () {
		it('Should distribute ERC721 tokens correctly', async function () {
			...
			// Check ownership
			expect(await this.erc721.ownerOf(1)).to.equal(
				await addr1.getAddress()
			);
			expect(await this.erc721.ownerOf(2)).to.equal(
				await addr2.getAddress()
			);
			expect(await this.erc721.ownerOf(3)).to.equal(
				await addr3.getAddress()
			);
		});
	});
	...
	describe('ERC1155 Distribution', function () {
		it('Should distribute ERC1155 tokens correctly', async function () {
			...
			// Approve distributor
			await this.erc1155.setApprovalForAll(
				await distributor.getAddress(),
				true
			);
			...
			for (const recipient of recipients) {
				expect(
					await this.erc1155.balanceOf(recipient, tokenId)
				).to.equal(10);
			}
			expect(
